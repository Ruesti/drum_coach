...
**Scope-Hinweis aus `BERICHT_TRAINING_PROGRAM.md`** *(zum Zeitpunkt der ursprünglichen Audit-Erstellung, inzwischen überholt — siehe Nachtrag unten)*: Ein *zweiter*, strukturell anderer Generator (`TrainingProgram`/`ProgramPhase`/`ProgramDay`/`ExerciseBlock`, ein 12-Wochen-Programm) war in `BRIEF_TRAINING_PROGRAM.md`/`STICK_CONTROL_PROGRAM.md` geplant, aber zum damaligen Stand **nie implementiert** (grep auf diese Klassennamen — keine Treffer). Kein aktueller Konflikt, aber die *Absicht*, ein zweites paralleles Generator-System zu bauen, sollte bei der Abstimmung mit diesem Brief berücksichtigt werden — der Brief warnt explizit vor "drei parallelen Systemen".

> **Nachtrag (nach Merge von `feature/training-program-stick-control`, 5 neue Commits `33c34f2`–`7a9cd96`):** Das oben beschriebene zweite Generator-System **existiert jetzt** — `lib/features/program/models/training_program.dart` (`TrainingProgram`, `ProgramPhase`, `ProgramDay`, `ExerciseBlock`, exakt wie vorhergesagt), `lib/features/program/data/stick_control_program.dart` (die `const`-Curriculum-Definition), `lib/features/program/program_provider.dart` (Generator: expandiert die 84 `ProgramDay`s zur Laufzeit, nie hart gespeichert), `lib/features/program/program_screen.dart` + Dashboard-Einstieg. Korrektur zur ursprünglichen Vermutung: **keine eigene Isar-Collection pro Tag/Block** — einzig persistierter Zustand ist `CleanTempo` (`lib/data/local/models/clean_tempo.dart`), ein Row-pro-`exerciseKey`-Fortschrittswert (letztes sauberes Tempo), der wie `RudimentProgress` per String-Key auf ein bestehendes `Rudiment.id` verweist. Das ist strukturell identisch zur SM-2/BPM-Kopplung (Abschnitt oben) — auch dieses System ist bereits ID-verknüpft und generisch genug, um sauber an ein vereinheitlichtes `Exercise.id` anzudocken, ohne Umbau. Die im Brief gewarnten "drei parallelen Systeme" sind damit real vorhanden (SM-2-Tagesplanung, KI-Freitext-Generator, jetzt zusätzlich das Stick-Control-Curriculum) — alle drei sind aber unabhängig voneinander ID-generisch, was das Risiko einer späteren Konsolidierung senkt. Diese Koexistenz sollte explizit mit dem Brief-Autor abgeglichen werden, analog zu den bereits in Phase 0.5 geklärten Fragen.
